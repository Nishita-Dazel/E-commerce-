            ...
            <div className='w-[800px] mx-auto border rounded'>
                <div className="actual-receipt mt-4  p-10">
                    <div className='flex'>
                        ...
                        <h1 className='ml-auto text-5xl font-bold'>Invoice</h1>
                    </div>

                    <div className='my-10'> <UserInfo /></div>
                    <div className='relative overflow-x-auto mb-5'>
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-900 uppercase dark:text-gray-400">
                                <tr className='border-b-2 border-black text-lg'>
                                    <th scope="col" class="pr-6 py-2 ">Product name</th>
                                    <th scope="col" class="px-4 py-2 text-center">Quantity</th>
                                    <th scope="col" class="px-4 py-2 text-center">Unite Price</th>
                                    <th scope="col" class="pl-4 py-2 text-right">Total</th>
                                </tr>
                            </thead>
                            <tbody>
                                {data.map(({ id, name, qty, price }) => {
                                    return <InvoiceCard id={id} name={name} qty={qty} price={price} />
                                })}
                                <PaymentTotal />
                            </tbody>
                        </table>
