...
const Category = () => {
    ...
    return (
        <div className='relative'>
            ...
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-2 bg-gray-100'>
                {
                    data.map(({ id, name, image_url }) => {
                        return <CategoryCard id={id} name={name} image={image_url} />
                    })
                }
            </div>

            <Modal show={openModal} handleClose={() => setOpenModal(false)} size={'500px'} >
                <InputComponent type={'text'} onChange={(e) => { setValue({ ...value, name: e }) }} label={'Category Name'} />
